...
TEST_F(TextDiagnosticTest, EmitIncludeLocationHandlesAbsolutePaths) {
  ...
  td.emitIncludeLocation(full_source_loc, full_source_loc.getPresumedLoc(
                                              DiagOpts->ShowPresumedLoc));
  ASSERT_EQ(OS->str(), "In file included from "
                       "//this/is/absolutely/an/absolute/path/main.cpp:1:\n");
}
...
TEST_F(TextDiagnosticTest, emitImportLocationHandlesAbsolutePaths) {
  DiagOpts->AbsolutePath = true;
  auto td = BuildTextDiagnosticExposer();
  td.emitImportLocation(
      full_source_loc,
      full_source_loc.getPresumedLoc(DiagOpts->ShowPresumedLoc), "ModuleName");
  ASSERT_EQ(OS->str(), "In module 'ModuleName' imported from "
                       "//this/is/absolutely/an/absolute/path/main.cpp:1:\n");
}
...
TEST_F(TextDiagnosticTest, emitImportLocationHandlesRelativePaths) {
  ...
  ASSERT_EQ(
      OS->str(),
      "In module 'ModuleName' imported from an/absolute/path/main.cpp:1:\n");
}

TEST_F(TextDiagnosticTest, emitBuildingModuleLocationHandlesAbsolutePaths) {
  DiagOpts->AbsolutePath = true;
  auto td = BuildTextDiagnosticExposer();
  td.emitBuildingModuleLocation(
      full_source_loc,
      full_source_loc.getPresumedLoc(DiagOpts->ShowPresumedLoc), "ModuleName");
  ASSERT_EQ(OS->str(), "While building module 'ModuleName' imported from "
                       "//this/is/absolutely/an/absolute/path/main.cpp:1:\n");
}
